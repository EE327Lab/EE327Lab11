package com.example.wanglequn.lab11;

/**
 * Created by wanglequn on 3/25/16.
 */
...
// parameters.setFlashMode("off"); // 无闪光灯
    parameters.setPictureFormat(PixelFormat.JPEG); //Sets the image
        ...
        //the image format for preview picture,默认为 NV21
        /*【ImageFormat】JPEG/NV16(YCrCb format,used for Video)/NV21(YCrCb
...
// parameters.setPictureSize(3264, 1836); //nexus4 指定拍照图片的大小
        ...
        parameters.setPreviewSize(mPreviewHeight, mPreviewWidth); // 指定preview 的大小
// parameters.setPreviewFrameRate(20);
//这两个属性 如果这两个属性设置的和真实手机的不一样时,就会报错 // 横竖屏镜头自动调整
        if (this.getResources().getConfiguration().orientation != Configuration.ORIENTATION_LANDSCAPE) {
            parameters.set("orientation", "portrait"); //
            parameters.set("rotation", 90); // 镜头角度转 90 度(默认摄像头是横拍)
            mCamera.setDisplayOrientation(90); // 在 2.2 以上可以使用 } else
        }else// 如果是横屏
        {
